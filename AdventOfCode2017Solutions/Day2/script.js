...
function sumDifferenceBetweenMaxAndMin(){
	let sumOfRows = 0;
	let minimumElement = Number.MAX_VALUE;
	let maximumElement = puzzleInput[0][0];
	for(let i = 0; i < puzzleInput.length; i++){
		for(let j = 0; j < puzzleInput[i].length; j++){
			if(puzzleInput[i][j] < minimumElement){
				minimumElement = puzzleInput[i][j];
			}
			else if(puzzleInput[i][j] > maximumElement){
				maximumElement = puzzleInput[i][j];
			}
		}// end inner for
		sumOfRows += (maximumElement - minimumElement);
...
function sumEvenlyDivisble(){
	let sumOfRows = 0;
	let numA, numB;
	for(let i = 0; i < puzzleInput.length; i++){
		for(let j = 0; j < puzzleInput[i].length; j++){
			numA = puzzleInput[i][j];
			for(let k = j + 1; k < puzzleInput[i].length; k++){
				numB = puzzleInput[i][k];
				if(numA % numB === 0) {
					sumOfRows += numA / numB;
				}
				else if(numB % numA === 0){
...
const firstHalfSolution = sumDifferenceBetweenMaxAndMin();
const secondHalfSolution = sumEvenlyDivisble();

console.table({"Day 2 First Half Solution":firstHalfSolution, "Day 2 Second Half Solution":secondHalfSolution});